<div id="sidebar" class="user-sidebar">
        ...
        <?php if(!empty($_SESSION['username'])) {
            if($_SESSION['id'] === $row['id']) { ?>
				...
				<?php if ($_SESSION['level'] > 1) { ?>
					<div class="report-buttons-content">
						<?php print "<button type=\"button\" class=\"report-button\" onclick=\"javascript:location.href='/admin/manage/".htmlspecialchars($row['username'])."';\">Administrator Menu</button>"; ?>
					</div>
				<?php } ?>
            <?php } else { ?>
                <div class="user-action-content">
                    ...
                    <div class="report-buttons-content">
                        ...
                        <?php if ($_SESSION['level'] == 0) { ?> <button type="button" class="report-button" data-modal-open="#block-page">Block</button> <?php } ?>
						<?php 
                ...
                <div id="report-violator-page" class="dialog none" data-modal-types="report report-violation">
                    <div class="dialog-inner">
                        <div class="window">
                            <h1 class="window-title">Report this User to <?php print(SITE_NAME); ?> Administrators</h1>
                            <div class="window-body">
                                <form method="post" action="/users/<?=htmlspecialchars(urlencode($row['username']))?>/violators">
                                    <input type="hidden" name="token" value="<?=$_SESSION['token']?>">
                                    <p class="description">You are about to report a user for violating the <?php print(SITE_NAME); ?> Rules. This report will be sent to the <?php print(SITE_NAME); ?> administrators and not to the user you are reporting or any community staff.</p>
            ...
            if($_SESSION['level'] > 1) { ?>
                <div id="manage-page" class="dialog none">
                    <div class="dialog-inner">
                        <div class="window">
                            <h1 class="window-title">Administrator Menu</h1>
                            <div class="window-body">
                                <p class="description">Select a administrator management option below.</p>
                                <div class="form-buttons">
                                    <?php if($_SESSION['level'] > 1) { ?><input type="submit" class="post-button black-button sidebar-social-container" value="<?=$banned ? 'Unb' : 'B'?>an User" data-modal-open="#manage-ban-page"><?php } ?>
                                    <input type="submit" class="post-button black-button sidebar-social-container" value="Purge User" data-modal-open="#manage-purge-page">
                                    <input type="submit" class="post-button black-button sidebar-social-container" value="Delete User" data-modal-open="#manage-delete-page">
									<?php if ($_SESSION['level'] > 1) {
										print '<input type="button" onclick="window.location.href=\'/admin/server_status\';" class="black-button sidebar-social-container" value="Server Status">';
									}
									?>
    ...
    if(!empty($is_general)) { ?>
		...
		<li><a href="/info/faq" class="sidebar-menu-guide symbol"><span>Frequently Asked Questions (FAQ)</span></a></li>
                <li><a href="/info/rules" class="sidebar-menu-post symbol"><span>Site Rules</span></a></li>
                <li><a href="/info/contact" class="sidebar-menu-guide symbol"><span>Contact Us</span></a></li>
            </ul>
        </div>
    <?php } else { ?>
        <div class="sidebar-setting sidebar-container">
        ...
        <div class="sidebar-container sidebar-profile">
            ...
            <div class="user-data">
				...
				<?php
					...
					if (empty($row['pronounz']))
					{
						$pronounz = "Not Set";
					}
                    			else
					{
						$pronounz = $row['pronounz'];
					}
				?>
				...
				<div class="user-main-profile data-content">
                    ...
                    <div class="note birthday"><a href="<?=htmlspecialchars($website)?>"><?=htmlspecialchars($website)?></a></div>
                    <h4><span>Favorite Game Genres</span></h4>
                    <div class="note"><?=htmlspecialchars($genre)?></div>
		    <h4><span>Pronouns</span></h4>
                    <div class="note"><?=htmlspecialchars($pronounz)?></div>
                </div>
            </div>
        </div>
        <?php
