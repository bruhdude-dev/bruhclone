...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach($_POST as $key => &$value) {
        if(array_key_exists($key, $row) && $row[$key] !== $value) {
            switch($key) {
                ...
                case 'website':
                    if(mb_strlen($value) > 40) {
                        showJSONError(400, 1211337, 'Your website is too long.');
                    }
                    break;
                case 'genre':
                    if(mb_strlen($value) > 40) {
                        showJSONError(400, 1211337, 'Your favorite game genre list is too long.');
                    }
                    break;
                case 'pronounz':
                    if(mb_strlen($value) > 40) {
		...
		case 'profile_comment':
                ...
                case 'has_mh':
					...
					else
					{
						...
						$gravatar = (!empty($row['email']) ? GRAVATAR_URL . md5($row['email']) . '?s=' . GRAVATAR_IMAGE_SIZE : null);
                        if(empty($row['mh'])) {
                            $edits[] = 'avatar = NULL';
                            $sessionEdits['avatar'] = null;
                        } else {
                            $edits[] = 'avatar = "' . $db->real_escape_string($gravatar) . '"';
                            $sessionEdits['avatar'] = $gravatar;
                        }
					}
				case 'site_theme':
    ...
    ?><div class="main-column messages">
        <div class="post-list-outline">
            ...
            <form class="setting-form" action="/settings/profile" method="post">
                ...
                <ul class="settings-list">
					...
					<li>
                        ...
                        <input type="text" class="url-form" name="country" maxlength="40" value="<?=htmlspecialchars($row['country'])?>" placeholder="Country">
                    </li>
					<li>
                        <p class="settings-label">Birthday</p>
                        <input type="text" class="url-form" name="birthday" maxlength="20" value="<?=htmlspecialchars($row['birthday'])?>" placeholder="Birthday">
                    </li>
					<li>
                        <p class="settings-label">Website</p>
                        <input type="text" class="url-form" name="website" maxlength="40" value="<?=htmlspecialchars($row['website'])?>" placeholder="Website">
                    </li>
                    <li>
                        <p class="settings-label">Favorite Game Genres</p>
                        <input type="text" class="url-form" name="genre" maxlength="40" value="<?=htmlspecialchars($row['genre'])?>" placeholder="Genre 1 / Genre 2 / Genre 3">
                    </li>
			<li>
                    ...
                    <li class="setting-profile-comment">
                        ...
                        <p class="note">What you write here will appear on your profile. Feel free to write anything that doesn't violate <a href="/info/rules">bruhclone's rules</a>.</p>
                    </li>
		    ...
		    <embed src="https://rewrite.webs.nf/php/mii2.mp3" hidden="TRUE" autostart="TRUE">
                </div>
            </form>
        </div>
    </div><?php
